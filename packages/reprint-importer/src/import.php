...
class ImportClient
{
    ...
    private function resolve_pull_only_files_with_path_prefixes(array $only_raw): array
    {
        ...
        foreach ($sources as $path) {
            ...
            foreach ($sources as $other) {
                if (self::is_parent_dir($other, $path)) {
                    $covered = true;
                    break;
                }
            }
    ...
    /**
     * Whether $parent is a strict parent directory of $path.
     */
    private static function is_parent_dir(string $parent, string $path): bool
    {
        $parent = rtrim($parent, "/");
        $path = rtrim($path, "/");
